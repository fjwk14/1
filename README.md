...
npm test          # ユニットテスト(バリデーション・権限・AI・動画URL)
npm run build     # 本番ビルド
```

### テスト

- **ユニットテスト(63件のうち38件)**: `npm test` で常時実行。DB不要。
- **RLS統合テスト(25件)**: 実PostgreSQLに対して権限分離を検証。
  `DATABASE_URL` が設定されている時のみ実行され、未設定ならスキップされます。

  ```bash
  # Supabase CLI がある場合
  supabase start && supabase db reset
  DATABASE_URL=postgres://postgres:postgres@localhost:54322/postgres npm test

  # Docker が使えない環境(素のPostgreSQL)の場合
  createdb kgtv
  psql -d kgtv -f scripts/local-supabase-shim.sql   # Supabase相当のロール/authスキーマを用意
  psql -d kgtv -f supabase/migrations/0001_init.sql
  psql -d kgtv -f supabase/seed.sql
  DATABASE_URL=postgres://postgres:postgres@localhost:5432/kgtv npm test
  ```

- **ブラウザE2E(16ステップ / 任意)**: `scripts/e2e.mjs` はサインアップ→チーム作成→
  試合→クリップ→タグ→コメント→集計→AIレポート→権限→データ分離を実ブラウザで通します。
  Docker無しでもローカル検証できるよう `scripts/dev-supabase-shim.mjs`
  (実PostgreSQL+RLSをバックにしたSupabase互換のAuth+RESTサーバー)を同梱しています。
  これらは開発・検証専用で、本番のSupabase Cloudでは使いません。
...
src/
  lib/
    ...
    video.ts                 # タイムスタンプ付き動画URL生成(危険スキーム防止)
scripts/
    local-supabase-shim.sql  # 素のPostgreSQLでRLSを検証するためのシム
    dev-supabase-shim.mjs    # Docker無しE2E用のSupabase互換devサーバー
    e2e.mjs                  # Playwrightによる16ステップのE2E
  app/
...
### 本番(Supabase Cloud)での注意

- `supabase/config.toml` の `enable_confirmations` などはローカル(`supabase start`)専用です。
  本番のメール確認・サインアップ可否は Supabase ダッシュボードの Authentication 設定で行います。
  メール確認を有効にする場合、サインアップ後はセッションが張られないため、
  アプリは「確認メールを送信しました」と案内します(`src/app/login/actions.ts`)。
- サインアップは誰でも可能で、チーム作成者が自動的にそのチームの admin になります。
  部外者に無制限にチームを作らせたくない場合は、ダッシュボードでサインアップを制限するか、
  将来的に招待制へ拡張してください。
